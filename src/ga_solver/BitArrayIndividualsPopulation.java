 ...
 *
 */
public class BitArrayIndividualsPopulation implements Population<Integer> {
	...
	public BitArrayIndividualsPopulation( int problemSize, int popSize , int childs, double pc, double pm, int iterMax){
		...
		performance = new Integer[iterMax];
	...
	public void initialization(){
		for(int i = 0; i<populationSize ;i++){
			Individual  ind = new BitArrayGenotype(problemSize);
			ind.setGeneration(0);
			population.add(ind);
	...
	public boolean hasBestIndividual(){
		...
		while(i < currentPopulation.size() && isOk==false){
			...
			if((Integer) ind.getFitness() == problemSize){
				isOk = true;
			}
			i++;
		}
	...
	public Individual[] monoPointCrossOver(Individual i1, Individual i2){
		...
		Individual children1 = new BitArrayGenotype(problemSize);
		Individual children2 = new BitArrayGenotype(problemSize);
	...
	public void bestSelection(){
		...
		int bestFitness =(Integer) currentPopulation.get(0).getFitness();
		int i = 0;
		boolean ok =true;
		while(i<currentPopulation.size() && ok){
			if ((Integer)currentPopulation.get(i).getFitness()==bestFitness){
	...
	public Integer bestFitness(){
		...
		for(int i=0 ; i <currentPopulation.size() ; i++){
			Individual ind = currentPopulation.get(i);
			int fitness = (Integer) ind.getFitness();
	 ...
	 */
	public Integer[] run(int selection, int crossover, int mutation, int insertion){
		int stepCounter = 0;
	 ...
	 */
	public Integer[] runByAdaptativeWheel(int selection, int crossover, int insertion){
		int stepCounter = 0;
		//boolean isOk = false;
	 ...
	 * @return
	 */
	 public static double[] average(ArrayList<Integer[]> executions, int iterMax) {
	 ...
	 public void islandEvolution(int selection, int crossover, int mutation, int insertion, int generation){
		 ...
		 if(this.currentPopulation.size()>=2){
			...
			// choix de l'operateur  et application de la  mutation
			if (probableChoice(mutationProba)){
				fitnessBefore[0]= (Integer) childs[0].getFitness(); fitnessBefore[1]= (Integer) childs[1].getFitness();
				...
				fitnessAfter[0]= (Integer) childs[0].getFitness(); fitnessAfter[1]= (Integer) childs[1].getFitness();
	...
	public void updateFitnessBeforeEvolution(){
		 ...
		 for(int i =0; i<currentPopulation.size();i++){
			 ...
			 fitnessBeforeEvolution[1][i]= (Integer) currentPopulation.get(i).getFitness();
		 }
	 ...
	 public void updateFitnessAfterEvolution(){
		 ...
		 for(int i =0; i<currentPopulation.size();i++){
			 ...
			 fitnessAfterEvolution[1][i]= (Integer) currentPopulation.get(i).getFitness();
		 }
	...
	public static void main(String args[]) throws IOException{
		ArrayList< Integer[]> executions = new  ArrayList< Integer[]>();
		...
		if(args.length==10){
			...
			Population s = new BitArrayIndividualsPopulation(size,popupaltionSize,2,pc,pm,max);
			if(mutationType>=0) {
				for (int i = 0; i< tests; i++) {
					 s = new BitArrayIndividualsPopulation(size,popupaltionSize,2,pc,pm,max);
					 executions.add( (Integer[]) s.run(selectionType, crossoverType, mutationType, insertionType));
				}
				...
				for (int i = 0; i< tests; i++) {
					 ...
					 executions.add( (Integer[]) s.runByAdaptativeWheel(selectionType, crossoverType, insertionType));
